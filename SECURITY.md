...
- **Copied files from anywhere on the computer** (Super+Alt+D after copying in the file manager): only a request with the local owner token can name paths outside the Vault, so no browser session, not even an admin's, can make Vault send an arbitrary file. Symlinks are refused, private locations (`~/.ssh`, `~/.gnupg`, `~/.password-store`, keyrings, Vault's own config, `/etc`, `/proc`, `/sys`, `/dev`, `/run`, `/boot`, `/root`) are never sent, nor is a folder containing one (e.g. the whole home folder). The clipboard is only read, when you press the shortcut; the window shows what will be sent before any phone scans it.
